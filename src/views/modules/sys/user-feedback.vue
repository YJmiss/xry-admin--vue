<template>
  <div class="container">
    ...
    <el-table :data="dataList" border style="width: 100%">
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row.nickname">{{scope.row.nickname}}&nbsp;/&nbsp;{{scope.row.phone}}</span>
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            size="small"
            type="text"
            v-popover:replyPopover
            @click="showDetail(scope.row.feedback_info)"
          >{{scope.row.feedback_info}}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" header-align="center" align="canter">
        <template slot-scope="scope" prop="check_status">
          <el-button
            v-if="isAuth('xry:feedback:detail')"
            type="primary"
            size="small"
            @click="viewFeedbackInfo(scope.row.id)"
          >反馈详情</el-button>
          <el-button
            v-if="isAuth('xry:question:delete')"
            type="danger"
            size="small"
            icon="el-icon-delete"
            circle
          ...
          ></el-button>
          <el-button
            v-if="isAuth('xry:feedback:replyHandle')"
            type="primary"
            size="small"
            icon="el-icon-edit"
            @click="replyHandle(scope.row.id)"
            v-show="scope.row.check_status ===0"
          >回复反馈</el-button>
          <el-button
            v-if="isAuth('xry:feedback:replyHandle')"
            type="success"
            ...
            icon="el-icon-edit"
            @click="replyHandle(scope.row.id)"
            v-show="scope.row.check_status ===1"
          >已回复</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 弹窗, 查看反馈详情 -->
    <feedback-detail
      v-show="feedbackDetailVisible"
      ref="feedbackDetail"
      @refreshDataList="getDataList"
    ></feedback-detail>
    <!-- 弹窗, 填写回复内容 -->
    <reply-add-or-view
      v-show="replyAddOrViewVisible"
      ref="replyAddOrView"
      @refreshDataList="getDataList"
    ></reply-add-or-view>
  </div>
